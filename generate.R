...
if(!file.exists("./CourseraSwiftKey")){dir.create("./CourseraSwiftKey")}
if(!file.exists("./CourseraSwiftKey/Coursera-SwiftKey.zip")){
  download.file(Url,destfile="./CourseraSwiftKey/Coursera-SwiftKey.zip",mode = "wb")
}
unzip(zipfile = "./CourseraSwiftKey/Coursera-SwiftKey.zip", exdir = "./CourseraSwiftKey")

#Select files
file_path<-"./CourseraSwiftKey/final/en_US/"
file_blogs <- "en_US.blogs.txt"
file_news  <- "en_US.news.txt"
file_twitter <- "en_US.twitter.txt"

blogs_file <- paste(file_path, file_blogs,sep="")
news_file <- paste(file_path, file_news,sep="")
twitter_file <-  paste(file_path, file_twitter, sep="")
...
# remove profanity
url2<-"http://www.bannedwordlist.com/lists/swearWords.csv"
if(!file.exists("./CourseraSwiftKey/swearWords.csv")){
  download.file(url2,destfile="./CourseraSwiftKey/swearWords.csv")
}
swear_words <- read_delim("./CourseraSwiftKey/swearWords.csv", delim = "\n", col_names = FALSE)
#swear_words <- read_delim("./CourseraSwiftKey/swearWords.csv", delim = "\n", col_names = FALSE)
